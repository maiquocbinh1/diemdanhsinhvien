...
class Subject:
    def __init__(self,root):
        self.root=root
        # Fit to current screen to avoid broken absolute layouts on smaller displays / DPI scaling
        ...
        self.heading = Label(self.root, text=self.txt, font=("yu gothic ui", 28, "bold"), bg="white", fg="black",
                             bd=5, relief=FLAT)
        # Wider title to prevent truncation
        ...
        left_x, left_y, left_h = 20, 5, 320
        left_w = max(900, main_w - left_x * 2)
        Left_frame.place(x=left_x, y=left_y, width=left_w, height=left_h)
        ...
        Right_frame = LabelFrame(Left_frame, bg="white",bd=0,
                                 font=("times new roman", 12, "bold"))
        Right_frame.place(x=530, y=5, width=max(600, left_w - 550), height=300)
        ...
        under_y, under_h = 335, min(320, max(260, main_h - 345))
        gap = 10
        under_w_each = (left_w - gap) // 2
        Underleft_frame.place(x=20, y=under_y, width=under_w_each, height=under_h)
        ...
        Underright_frame.place(x=20 + under_w_each + gap, y=under_y, width=under_w_each, height=under_h)
