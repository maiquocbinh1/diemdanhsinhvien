...
class Attendance:
    ...
    def __init__(self,root):
        ...
        left_x, top_y = 20, 5
        gap = 10
        left_w = max(420, int(main_w * 0.34))
        left_h = max(420, main_h - top_y - 5)
        ...
        left_inside_frame.place(x=0, y=60, width=max(300, left_w - 20), height=max(260, left_h - 80))
        ...
        label_Update_att.place(x=max(10, int(left_w * 0.2)), y=max(260, left_h - 160), width=max(220, int(left_w * 0.55)), height=45)
        update_btn = Button(Left_frame, text="Xóa", command=self.delete_data,
                            font=("times new roman", 14, "bold"),
                            bg="red", fg="white", width=250)
        update_btn.place(x=max(10, int(left_w * 0.2)), y=max(310, left_h - 80), width=max(220, int(left_w * 0.55)), height=45)


        btn_frame = Frame(left_inside_frame, bg="white")
        btn_frame.place(x=0, y=max(200, left_h - 330), width=max(280, left_w - 10), height=105)
        ...
        table_frame.place(x=5, y=55, width=max(360, right_w - 10), height=max(200, right_h - 70))
        ...
        self.AttendanceReportTable.column("id", width=120, stretch=True)
        self.AttendanceReportTable.column("idsv", width=120, stretch=True)
        self.AttendanceReportTable.column("name", width=180, stretch=True)
        self.AttendanceReportTable.column("class", width=120, stretch=True)
        self.AttendanceReportTable.column("time_in", width=110, stretch=True)
        self.AttendanceReportTable.column("time_out", width=110, stretch=True)
        self.AttendanceReportTable.column("date", width=140, stretch=True)
        self.AttendanceReportTable.column("attendance", width=120, stretch=True)
        self.AttendanceReportTable.column("lesson", width=120, stretch=True)
