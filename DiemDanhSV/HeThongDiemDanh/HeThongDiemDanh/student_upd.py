...
class Student:
    def __init__(self,root):
        ...
        img3 = img3.resize((win_w, win_h), PIL.Image.ANTIALIAS)
        self.photoimg3 = ImageTk.PhotoImage(img3)

        bg_img = Label(self.root, image=self.photoimg3)
        bg_img.place(x=0, y=0, width=win_w, height=win_h)
        ...
        self.heading = Label(self.root, text=self.txt, font=("yu gothic ui", 28, "bold"), bg="white", fg="black",
                             bd=5, relief=FLAT)
        heading_w = min(980, win_w - 260)
        self.heading.place(x=(win_w - heading_w) // 2, y=22, width=heading_w)
        self.slider()
        self.heading_color()

        main_frame = Frame(bg_img, bd=2, bg="white")
        main_x, main_y = 23, 102
        main_w = max(1000, win_w - (main_x * 2))
        main_h = max(500, win_h - main_y - 17)
        main_frame.place(x=main_x, y=main_y, width=main_w, height=main_h)
        ...
        left_x, top_y = 10, 10
        gap = 10
        left_w = max(520, int(main_w * 0.48))
        left_h = max(420, main_h - (top_y * 2))
        Left_frame.place(x=left_x, y=top_y, width=left_w, height=left_h)
        ...
        right_w = max(520, main_w - right_x - left_x)
        # use full height so we can stack (table + class mgmt) like the desired layout
        right_h = left_h
        Right_frame.place(x=right_x, y=top_y, width=right_w, height=right_h)


        #Search_frame
        search_frame = LabelFrame(Right_frame, bd=2, bg="white", relief=RIDGE, text="Hệ Thống Tìm kiếm",
                                         font=("yu gothic ui", 13, "bold"))
        search_frame.place(x=5, y=5, width=max(360, right_w - 10), height=70)
        ...
        #table-------frame (upper area)
        table_frame = Frame(Right_frame, bd=2, bg="white", relief=RIDGE)
        table_h = max(180, int(right_h * 0.45))
        table_frame.place(x=5, y=85, width=max(360, right_w - 10), height=table_h)
        ...
        # allow table to adapt; keep min widths but let it stretch
        self.student_table.column("id", width=110, stretch=True)
        self.student_table.column("dep", width=140, stretch=True)
        self.student_table.column("course", width=160, stretch=True)
        self.student_table.column("year", width=90, stretch=True)
        self.student_table.column("sem", width=100, stretch=True)
        self.student_table.column("name", width=180, stretch=True)
        self.student_table.column("div", width=120, stretch=True)
        self.student_table.column("roll", width=120, stretch=True)
        self.student_table.column("gender", width=90, stretch=True)
        self.student_table.column("dob", width=110, stretch=True)
        self.student_table.column("email", width=180, stretch=True)
        self.student_table.column("phone", width=120, stretch=True)
        self.student_table.column("address", width=180, stretch=True)
        self.student_table.column("photo", width=120, stretch=True)
        ...
        self.getNextid()
        #===============================bottomright-Class==============================
        under_y = 85 + table_h + 10
        under_h = max(200, right_h - under_y - 5)
        Underright_frame = LabelFrame(Right_frame, bd=2, bg="white", relief=RIDGE,
                                      font=("yu gothic ui", 12, "bold"))
        Underright_frame.place(x=5, y=under_y, width=max(360, right_w - 10), height=under_h)

        label_studentsb = Label(Underright_frame, bg="white", fg="red2", text="Quản lý lớp học",
                                font=("yu gothic ui", 14, "bold"))
        label_studentsb.place(x=0, y=1, width=max(300, right_w - 30), height=35)
        ...
        btn_framestd.place(x=20, y=max(140, under_h - 70), width=min(520, max(360, int((right_w - 30) * 0.65))), height=55)
        ...
        tablestd_w = max(210, int((right_w - 30) * 0.30))
        tablestd_frame.place(x=max(20, int((right_w - 30) * 0.68)), y=40, width=tablestd_w, height=max(160, under_h - 55))
